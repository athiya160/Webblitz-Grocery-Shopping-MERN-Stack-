...
const userRegister = async (req, res) => {
  try {
    ...
    const validatedFamilyDetails = familyDetails.map(member => ({
      name: member.name,
      relation: member.relation,
      age: member.age,
      sugar: member.sugar,
      bp: member.bp,
      creatinine: member.creatinine,
      glucose: member.glucose,
      skinthickness: member.skinthickness,
      insulin: member.insulin,
      bmi: member.bmi,
    }));
    ...
    const newUser = await userModel.create({
      ...
      smoke,
      alcohol,
      familyDetails: validatedFamilyDetails
    });
